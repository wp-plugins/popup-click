<?php
/**
 * Pop-Up PRO CC - Click
 *
 * @package   CcPopUpProClick
 * @author    Chop-Chop.org <shop@chop-chop.org>
 * @license   GPL-2.0+
 * @link      https://shop.chop-chop.org
 * @copyright 2014 
 */

if ( ! class_exists( 'ChChPUCFPeview' ) )
    require_once( CHCH_PUCF_PLUGIN_DIR. 'admin/includes/chch-pucf-preview.php' );

if ( ! class_exists( 'ChChPCFTemplate' ) )
    require_once( CHCH_PUCF_PLUGIN_DIR . 'public/includes/chch-pucf-template.php' );
/**
 * @package CcPopUpProClick
 * @author 	Chop-Chop.org <shop@chop-chop.org>
 */
 

class ChChPopUpClickAdmin {

	/**
	 * Instance of this class.
	 *
	 * @since    1.0.0
	 *
	 * @var      object
	 */
	protected static $instance = null;
	
	function __construct() {
		$this->plugin = ChChPopUpClick::get_instance(); 
		$this->plugin_slug = $this->plugin->get_plugin_slug();
		
		// Register Post Type and messages
		add_action( 'init', array( $this, 'chch_pucf_register_post_type' ) );
		// Customize the columns in the popup list.
		add_filter( 'manage_chch-pucf_posts_columns',array( $this, 'chch_pucf_custom_pop_up_columns') );
		// Returns the content for the custom columns.
		add_action( 'manage_chch-pucf_posts_custom_column',array( $this, 'chch_pucf_manage_custom_pop_up_columns' ),10, 2); 
		add_filter( 'post_updated_messages',  array( $this, 'chch_pucf_post_type_messages') );
		
		// Register Post Type Meta Boxes and fields
		add_action( 'init', array( $this, 'chch_pucf_initialize_cmb_meta_boxes'), 9999 );
		add_action( 'cmb_render_chch_pucf_pages_select', array( $this, 'chch_pucf_render_pages_select'), 10, 5  ); 
		add_action( 'cmb_render_pucf_cookie_select', array( $this, 'chch_pucf_render_cookie_select'), 10, 5  );
		add_action( 'add_meta_boxes_chch-pucf', array( $this, 'chch_pucf_metabox' ));
		add_filter( 'cmb_meta_boxes', array( $this, 'chch_pucf_post_type_metaboxes') );
		
		// remove help tabs
		add_filter( 'contextual_help', array($this,'chch_pucf_remove_help_tabs'), 999, 3 );
		add_filter( 'screen_options_show_screen', '__return_false');
		
		// Save Post Data
		add_action( 'save_post', array( $this, 'chch_pucf_save_pop_up_meta'), 10, 3 ); 
		 
		// Templates view
		add_action('edit_form_after_title',array( $this, 'chch_pucf_templates_view' ));
		
		// admin scripts and AJAX
		add_action( 'admin_init', array( $this,'chch_pucf_tinymce_event') );
		add_action( 'admin_head', array( $this, 'chch_pucf_admin_head_scripts') );    
		add_action( 'admin_enqueue_scripts', array( $this, 'chch_pucf_enqueue_admin_scripts' ));
		add_action( 'wp_ajax_chch_pucf_load_preview_module', array( $this, 'chch_pucf_load_preview_module'  ));  
		 
	} 
	
	
	/**
	 * Return an instance of this class.
	 *
	 * @since     1.0.0
	 *
	 * @return    object    A single instance of this class.
	 */
	public static function get_instance() {

		// If the single instance hasn't been set, set it now.
		if ( null == self::$instance ) {
			self::$instance = new self;
		}

		return self::$instance;
	}
	
	
	/**
	 * Register Custom Post Type
	 *
	 * @since    0.1.0
	 */
	public function chch_pucf_register_post_type() {
		
		$domain = $this->plugin_slug;
		
		$labels = array(
			'name'                => _x( 'Pop-Up CC Click Free', 'Post Type General Name', $domain),
			'singular_name'       => _x( 'Pop-Up Click Free', 'Post Type Singular Name', $domain),
			'menu_name'           => __( 'Pop-Up CC Click Free', $domain),
			'parent_item_colon'   => __( 'Parent Item:', $domain),
			'all_items'           => __( 'Pop-Ups CC Click Free', $domain),
			'view_item'           => __( 'View Item', $domain),
			'add_new_item'        => __( 'Add New Pop-Up Click Free', $domain),
			'add_new'             => __( 'Add New Pop-Up Click Free', $domain),
			'edit_item'           => __( 'Edit Pop-Up Click Free', $domain),
			'update_item'         => __( 'Update Pop-Up Click Free', $domain),
			'search_items'        => __( 'Search Pop-Up Click Free', $domain),
			'not_found'           => __( 'Not found', $domain),
			'not_found_in_trash'  => __( 'No Pop-Up Close found in Trash', $domain),
		);
 

		$args = array(
			'label'               => __( 'Pop-Up Click Free', $domain),
			'description'         => __( '', $domain),
			'labels'              => $labels,
			'supports'            => array( 'title' ),
			'hierarchical'        => false,
			'public'              => false,
			'show_ui'             => true,
			'show_in_menu'        => true,
			'show_in_nav_menus'   => true,
			'show_in_admin_bar'   => false,
			'menu_position'       => 65, 
			'can_export'          => true,
			'has_archive'         => false,
			'exclude_from_search' => true,
			'publicly_queryable'  => false,
			'rewrite'             => false
		);
		register_post_type( 'chch-pucf', $args );
	}
	
	
	/**
	 * Register custom post list columns
	 *
	 * @since     1.0.0
	 * 
	 */
	function chch_pucf_custom_pop_up_columns($defaults) {
		$defaults['chch_pucf_status'] = __('Active',$this->plugin_slug); 
		$defaults['chch_pucf_template'] = __('Template',$this->plugin_slug);
		
		return $defaults;
	}
	
	
	/**
	 * Fill columns in Pop-ups list
	 *
	 * @since     1.0.0  
	 */
	function chch_pucf_manage_custom_pop_up_columns($column, $post_id) {
		global $post;
		if ($column === 'chch_pucf_status') {
			echo ucfirst(get_post_meta($post_id,'_chch_pucf_status', true));
		}
		 
		
		if ($column === 'chch_pucf_template') {
			echo ucfirst(get_post_meta($post_id,'_chch_pucf_template', true));
		}
	}
	
	
	/**
	 * Pop-Ups update messages. 
	 *
	 * @param array $messages Existing post update messages.
	 *
	 * @return array Amended post update messages with new CPT update messages.
	 */
	function chch_pucf_post_type_messages( $messages ) {
		$post             = get_post();
		$post_type        = get_post_type( $post );
		$post_type_object = get_post_type_object( $post_type );
	
		$messages['chch-pucf'] = array(
			0  => '', // Unused. Messages start at index 1.
			1  => __( 'Pop-Up updated.', $this->plugin_slug ),
			2  => __( 'Custom field updated.', $this->plugin_slug),
			3  => __( 'Custom field deleted.',$this->plugin_slug),
			4  => __( 'Pop-Up updated.', $this->plugin_slug ), 
			5  => isset( $_GET['revision'] ) ? sprintf( __( 'Pop-Up restored to revision from %s', $this->plugin_slug ), wp_post_revision_title( (int) $_GET['revision'], false ) ) : false,
			6  => __( 'Pop-Up published.', $this->plugin_slug ),
			7  => __( 'Pop-Up saved.', $this->plugin_slug ),
			8  => __( 'Pop-Up submitted.', $this->plugin_slug ),
			9  => sprintf(
				__( 'Pop-Up scheduled for: <strong>%1$s</strong>.', $this->plugin_slug ), 
				date_i18n( __( 'M j, Y @ G:i', $this->plugin_slug ), strtotime( $post->post_date ) )
			),
			10 => __( 'Pop-Up draft updated.', $this->plugin_slug )
		);
	
		if ( $post_type_object->publicly_queryable ) {
			$permalink = get_permalink( $post->ID );
	
			$view_link = sprintf( ' <a href="%s">%s</a>', esc_url( $permalink ), __( 'View Pop-Up',  $this->plugin_slug ) );
			$messages[ $post_type ][1] .= $view_link;
			$messages[ $post_type ][6] .= $view_link;
			$messages[ $post_type ][9] .= $view_link;
	
			$preview_permalink = add_query_arg( 'preview', 'true', $permalink );
			$preview_link = sprintf( ' <a target="_blank" href="%s">%s</a>', esc_url( $preview_permalink ), __( 'Preview Pop-Up',  $this->plugin_slug ) );
			$messages[ $post_type ][8]  .= $preview_link;
			$messages[ $post_type ][10] .= $preview_link;
		}
	
		return $messages;
	}
	
	
	/**
	 * Initialize Custom Metaboxes Class
	 *
	 * @since  0.1.0 
	 */
	function chch_pucf_initialize_cmb_meta_boxes() {
 		if ( ! class_exists( 'cmb_Meta_Box' ) )
			require_once( CHCH_PUCF_PLUGIN_DIR . 'admin/includes/Custom-Metaboxes-and-Fields-for-WordPress-master/init.php' ); 
	}
	
	/**
	 * Return a cookie_select field for CMB
	 *
	 * @since     1.0.0
	 * 
	 */
	function chch_pucf_render_cookie_select( $field_args, $escaped_value, $object_id, $object_type, $field_type_object ) {
		$cookie_expire = array(
			'refresh' => 'Click',
			'session' => 'Session',
			'Day' => 'Day (Available in Pro)',
			'Week' => 'Week (Available in Pro)',
			'Month' => 'Month (Available in Pro)',
			'Year' => 'Year (Available in Pro)',	
		);
		?>
		
		<select class="cmb_select" name="<?php echo $field_args['_name']; ?>" id="<?php echo $field_args['_id']; ?>">	
		
		<?php
			foreach($cookie_expire as $value => $title):
				$selected = '';
				$disable = '';
				
				if(!empty($escaped_value)){
					if($value == $escaped_value) {
						$selected = 'selected';	
					} 
				}
				
				if($value != 'refresh' && $value != 'session') {
					$disable = 'disabled';	
				}
				
			 	echo '<option value="'.$value.'" '.$selected .' '.$disable.'>'.$title.'</option>';
			endforeach
		 ?>
		 
		</select> <a href="http://ch-ch.org/puipro" target="_blank">Get Pro</a><br />
				 
		<?php  
		echo $field_type_object->_desc( true );   
	}
	
	/**
	 * Return a pages_select field for CMB
	 *
	 * @since     1.0.0
	 * 
	 */
	function chch_pucf_render_pages_select( $field_args, $escaped_value, $object_id, $object_type, $field_type_object ) {
		$all_pages = $this->get_all_pages();
		 ?>
		<select class="cmb_select" name="<?php echo $field_args['_name']; ?>[]" id="<?php echo $field_args['_id']; ?>" multiple="multiple">	
		<?php 
			$selected = '';
			if(!empty($escaped_value) && is_array($escaped_value)){
				if(in_array( 'chch_home',$escaped_value)) {
					$selected = 'selected';	
				}
			}
		?>
		<option value="chch_home" <?php echo $selected ?>>Home (Latest Posts)</option>
		<?php
			foreach($all_pages as $value => $title):
				$selected = '';
				if(!empty($escaped_value)){
					if(in_array( $value,$escaped_value)) {
						$selected = 'selected';	
					} 
				}
			 	echo '<option value="'.$value.'" '.$selected .'>'.$title.'</option>	';
			endforeach
		 ?>
			</select>	 
		<?php  
		echo $field_type_object->_desc( true ); 
	}
	
	
	/**
	 * Get all pages for CMB select pages field
	 *
	 * @since  0.1.0 
	 */
	private function get_all_pages() {
		
		$args = array(
		   'public'   => true,
		   '_builtin' => true
		);
		
		$post_types = get_post_types( $args );
		
        $args = array(
			'post_type' => $post_types,
			'posts_per_page' => -1, 
			'orderby' => 'title',
			'order' => 'ASC'
		);
		
		$post_list = get_posts($args);
		
		$all_posts = array();
		
		if($post_list):
			foreach($post_list as $post):
				$all_posts[$post->ID] = get_the_title($post->ID);
			endforeach;
		endif;
		
        return $all_posts; 
	}
	
	
	/**
	 * Register custom metaboxes
	 *
	 * @since  1.0.0 
	 */
	function chch_pucf_post_type_metaboxes( array $meta_boxes ) {
		
		$domain = $this->plugin_slug; 
		$prefix = '_chch_pucf_';
	
		/**
		 * GENERAL OPTIONS
		 */
		$meta_boxes['chch-pucf-metabox-general'] = array(
			'id'         => 'chch-pucf-metabox-general',
			'title'      => __( 'GENERAL', $domain ),
			'pages'      => array( 'chch-pucf'), 
			'context'    => 'normal',
			'priority'   => 'low',
			'show_names' => true,  
			'fields'     => array( 
				
				array(
					'name'    => __( 'Pop-up Status', $domain ),
					'desc'    => __( 'Enable or disable the pop-up.', $domain  ),
					'id'      => $prefix . 'status',
					'type'    => 'radio_inline',
					'std'	=> 'yes',
					'options' => array(
						'yes' => __( 'Turned ON', $domain ),
						'no'   => __( 'Turned OFF', $domain ), 
					),
				),
				array(
					'name' => __( 'Show on mobile devices?', $domain  ),
					'desc'    => __( 'The pop-up will be visible on mobile devices.', $domain  ),
					'id'   => $prefix . 'show_on_mobile',
					'type' => 'checkbox',
				),
				array(
					'name' => __( 'Show only on mobile devices?', $domain  ),
					'desc'    => __( 'The pop-up will be visible on mobile devices only.', $domain  ),
					'id'   => $prefix . 'show_only_on_mobile',
					'type' => 'checkbox',
				), 
				array(
					'name' => __( 'Show once per', $domain  ),
					'desc'    => __( '', $domain  ),
					'id'   => $prefix . 'show_only_once',
					'type' => 'pucf_cookie_select', 
					'default' => 'refresh',
					
				),  
			),

		); 	
	
		
		/**
		 * DISPLAY CONTROL
		 */
		$meta_boxes['chch-pucf-metabox-control'] = array(
			'id'         => 'chch-pucf-metabox-control',
			'title'      => __( 'Display Control'),
			'pages'      => array( 'chch-pucf', ), 
			'context'    => 'normal',
			'priority'   => 'low',
			'show_names' => true,  
			'fields'     => array( 
				array(
					'name' => __( 'By Role:', $domain  ),
					'desc'    => __( 'Decide who will see the pop-up.', $domain  ),
					'id'   => $prefix . 'role',
					'type' => 'radio',
					'options' => array(
						'all' => __( 'All', $domain  ),
						'unlogged' => __( 'Show to unlogged users', $domain  ),
						'logged' => __( 'Show to logged in users', $domain  ),
					),
					'default' => 'all'
				),
				array(
					'name' => __( 'Disable on:', $domain  ),
					'desc'    => __( 'Decide on which pages the pop-up will not be visible.<br>Hold the ctrl key and click to select the pages which should not display the pop-up.', $domain  ),
					'id'   => $prefix . 'page',
					'type' => 'chch_pucf_pages_select',  
				), 
			),
		); 
		
		return $meta_boxes; 
	}
	
	/**
	 * Register custom metaboxes
	 *
	 * @since  0.1.0 
	 */
	public function chch_pucf_metabox( $post ) {
		remove_meta_box( 'slugdiv', 'chch-pucf', 'normal' ); 
		
		add_meta_box(
			'chch-pucf-metabox-click',
			__( 'Click Options', $this->plugin_slug ),
			array( $this, 'chch_pucf_render_click_metabox'),
			'chch-pucf'
			,'normal'
			,'high'
		);
		
		$post_boxes = array(
			'chch-pucf-metabox-general',
			'chch-pucf-metabox-content',
			'chch-pucf-metabox-control',
			'chch-pucf-metabox-click', 
		);	
		
		foreach($post_boxes as $post_box)
		{
			add_filter( 'postbox_classes_chch-pucf_'.$post_box,array( $this, 'chch_pucf_add_metabox_classes') );
		}
	}
	
	/**
	 * Remove help tabs from post view.
	 *
	 * @since     1.0.7
	 * 
	 */
	function chch_pucf_remove_help_tabs($old_help, $screen_id, $screen){
		if ( 'post' == $screen->base && 'chch-pucf' === $screen->post_type) {
			$screen->remove_help_tabs();
			return $old_help;
		}
	}
	
	
	/**
	 * Add metabox class for tabs
	 *
	 * @since  1.0.0 
	 */
	function chch_pucf_add_metabox_classes( $classes ) {
 		array_push( $classes, "cc-pu-tab-2 cc-pu-tab" );
		return $classes; 
	}
	
	/**
	 * View for Click Metabox
	 *
	 * @since  1.0.0 
	 */
	function chch_pucf_render_click_metabox($post) {
 		require_once( CHCH_PUCF_PLUGIN_DIR . 'admin/views/metabox/click-metabox.php' );
	}
	 
	
	
	/**
	 * Save Post Type Meta
	 *
	 * @since  0.1.0 
	 */
	function chch_pucf_save_pop_up_meta( $post_id, $post, $update ) { 
		if (
			!isset($_POST['chch_pucf_save_nonce']) 
			|| ! wp_verify_nonce($_POST['chch_pucf_save_nonce'],'chch_pucf_save_nonce_'.$post_id) 
		) 
		{
			return;
		}
		
		if(defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE) return;
		
		$slug = 'chch-pucf';
		
		if ( !is_admin()) {
			return;
		}
		  
		if ( $slug != $post->post_type ) {
			return;
		}
		
		$template =  $_REQUEST['_chch_pucf_template'];
		$template_base =  $_REQUEST['_chch_pucf_template_base'];
		
		update_post_meta( $post_id, '_chch_pucf_template', sanitize_text_field( $_REQUEST['_chch_pucf_template']) );
		update_post_meta( $post_id, '_chch_pucf_template_base', sanitize_text_field( $_REQUEST['_chch_pucf_template_base']) );
		
		 
		if(!empty($template) && !empty($template_base))
		{
			$template_data = array();
			
			if ( isset( $_REQUEST['_'.$template.'_size_custom'] ) ) { 
				$template_data['size'] = array(
					'custom' => 1
				);   
			} else {
				$template_data['size'] = array(
					'custom' => 0
				); 
			}
			
			$template_data['size']['width'] = sanitize_text_field($_REQUEST['_'.$template.'_size_width']); 
			$template_data['size']['height'] = sanitize_text_field($_REQUEST['_'.$template.'_size_height']);
			 
			$template_data['border']= array( 
				'radius' => $_REQUEST['_'.$template.'_border_radius']
			); 
			
			$template_data['overlay']= array( 
				'hide' => $_REQUEST['_'.$template.'_overlay_hide']
			);  
			  
			$p_array = array('</p>','<p>');
		 
			$template_data['contents']= array(
				'desc' => wp_kses_post(str_replace($p_array, '', $_REQUEST['_'.$template.'_contents_desc'])), 
				'video' => $_REQUEST['_'.$template.'_contents_video']
			);  
			
			update_post_meta( $post_id, '_chch_pucf_item', sanitize_text_field( $_REQUEST['_chch_pucf_item']) );
			   
			update_post_meta($post_id, '_'.$template.'_template_data', $template_data);	
		}
	}
	 
	
	
	/**
	 * Add Templates View
	 *
	 * @since  0.1.0 
	 */
	public function chch_pucf_templates_view( $post ) { 
		  
		$screen = get_current_screen();
		if ( 'post' == $screen->base && 'chch-pucf' === $screen->post_type) {
			
			include(CHCH_PUCF_PLUGIN_DIR . 'admin/views/templates.php' );
		}
	}
	
	
	/**
	 * Return list of templates
	 *
	 * @since     1.0.0
	 *
	 * @return    array - template list
	 */
	public function get_templates() {
		if ( ! class_exists( 'PluginMetaData' ) )
			require_once( CHCH_PUCF_PLUGIN_DIR . 'admin/includes/PluginMetaData.php' ); 
		$pmd = new PluginMetaData;
		$pmd->scan(CHCH_PUCF_PLUGIN_DIR . 'public/templates');
		return $pmd->plugin;
	}
	
	
	/**
	 * Register TinyMce event
	 *
	 * @since     1.0.0
	 * 
	 */
	function chch_pucf_tinymce_event() { 
		if (is_admin() && current_user_can( 'edit_posts' ) && current_user_can( 'edit_pages' ) ) {
			if ( get_bloginfo('version') < 3.9 ) { 
				add_filter( 'mce_external_plugins', array( $this, 'chch_pucf_tinymce_keyup_old') );
			} else
			{
				add_filter( 'mce_external_plugins', array( $this, 'chch_pucf_tinymce_keyup') );	 
			} 
		} 
	} 
	
	
	/**
	 * Add keyup to tineMce for WP version > 3.9
	 *
	 * @since     1.0.0
	 * 
	 */
	function chch_pucf_tinymce_keyup($plugin_array) { 
	 	$plugin_array['keyup_events'] = CHCH_PUCF_PLUGIN_URL .'admin/assets/js/chch-tinymce.js'; 
		return $plugin_array;
	}
	
	
	/**
	 * Add keyup to tineMce for WP version < 3.9
	 *
	 * @since     1.0.0
	 * 
	 */
	function chch_pucf_tinymce_keyup_old($plugin_array) { 
	 	$plugin_array['keyup_events'] = CHCH_PUCF_PLUGIN_URL .'admin/assets/js/chch-tinymce-old.js'; 
		return $plugin_array;
	}
	  
	
	/**
	 * Include google fonts
	 *
	 * @since  0.1.0 
	 */
	public function chch_pucf_admin_head_scripts() {
	 	$screen = get_current_screen();
		if ( 'post' == $screen->base && 'chch-pucf' === $screen->post_type) { 
			
			$js ="<link href='//fonts.googleapis.com/css?family=Playfair+Display:400,700,900|Lora:400,700|Open+Sans:400,300,700|Oswald:700,300|Roboto:400,700,300|Signika:400,700,300' rel='stylesheet' type='text/css'>";
			echo $js;
		}
	 } 
	 
	  
	/**
	 * Register and enqueue admin-specific style sheet.
	 
	 */
	public function chch_pucf_enqueue_admin_scripts() {

		$screen = get_current_screen();
		if ( 'post' == $screen->base && 'chch-pucf' === $screen->post_type) { 
			wp_enqueue_style('wp-color-picker' ); 
			wp_enqueue_script('jquery-ui-core');
			wp_enqueue_script('jquery-ui-slider'); 
			
			wp_enqueue_media();
			
			wp_enqueue_style( $this->plugin_slug .'-admin-styles', plugins_url( 'assets/css/admin.css', __FILE__ ), array(), ChChPopUpClick::VERSION );
			
			wp_enqueue_script( $this->plugin_slug .'-admin-scripts', plugins_url( 'assets/js/admin.js', __FILE__ ), array( 'jquery', 'wp-color-picker' ), ChChPopUpClick::VERSION );  
			wp_localize_script( $this->plugin_slug .'-admin-scripts', 'chch_pucf_ajax_object', array( 'ajaxUrl' => admin_url( 'admin-ajax.php' ),'chch_pop_up_url' => CHCH_PUCF_PLUGIN_URL) );
			
			wp_enqueue_style( $this->plugin_slug .'-ui', '//ajax.googleapis.com/ajax/libs/jqueryui/1.10.3/themes/smoothness/jquery-ui.min.css', null, ChChPopUpClick::VERSION,'all' );
			
			if(file_exists(CHCH_PUCF_PLUGIN_DIR . 'public/templates/css/defaults.css'))
			{
				wp_enqueue_style($this->plugin_slug .'_template_defaults', CHCH_PUCF_PLUGIN_URL . 'public/templates/css/defaults.css', null, ChChPopUpClick::VERSION, 'all');  
			}
			
			if(file_exists(CHCH_PUCF_PLUGIN_DIR . 'public/templates/css/fonts.css'))
			{
				wp_enqueue_style($this->plugin_slug .'_template_fonts', CHCH_PUCF_PLUGIN_URL . 'public/templates/css/fonts.css', null, ChChPopUpClick::VERSION, 'all');  
			} 
		}  
	}
	
	
	/**
	 * Load preview by ajax
	 
	 */
	function chch_pucf_load_preview_module() {
 
		$template = $_POST['template'];
		$template_base = $_POST['base'];
		$popup = $_POST['id'];
		
		$template = new ChChPCFTemplate($template,$template_base,$popup); 
		$template->get_template();	
		die();
	}
	 
}
